...
public class TestRunnerOpenEjbConfiguration {
    ...
    private static Logger logger = Logger.getLogger(TestRunnerOpenEjbConfiguration.class.getName());
    ...
    private static void loadDatasources(java.util.Properties p) {
        ...
        logger.info("Datasources carregados -> criando ALIAS JDNI");
        ...
        for (Object o : keys) {
            ...
            if (p.getProperty(dsKey).startsWith("new://Resource?type=DataSource")) {
                logger.info(String.format("Registrando Alias de JNDI para [java:%s]", dsKey));
                ...
                if (isOracle) {
                    ...
                    if (testQuery == null) {
                        logger.info(String.format("Registrando validationQuery para [java:%s]", dsKey));
    ...
    private static void loadProperties(String file, java.util.Properties p) {
        try (InputStream in = TestRunner.class.getClassLoader().getResourceAsStream(file)) {
            ...
            ds.load(in);
            p.putAll(ds);
            logger.info("propriedades carregadas -> [" + file + "] -> " + ds);
        } catch (Exception e) {
            logger.warning(String.format("Erro ao ler propriedades do arquivo [%s]: [%s]", file, e.getMessage()));
        }
    }
}
